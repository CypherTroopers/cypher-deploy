...
function Install-GoWindows {
    ...
    if (Test-Path -LiteralPath $preferred) {
        ...
        Write-Host "Go version differs from setup_cypherium.sh. Found: $currentVersion. Expected: go$Version."
    }
    if ($SkipInstall) {
...
function Assert-GoVersionSupported {
    ...
    if ($version -notmatch "go$([regex]::Escape($ExpectedVersion))") {
        throw "Go $ExpectedVersion is required to match setup_cypherium.sh. Found: $version"
    }
}
...
Invoke-NativeChecked -Command { & $MsysBash -lc "pacman -S --needed --noconfirm make cmake m4 texinfo ${mingwPrefix}-gcc ${mingwPrefix}-binutils ${mingwPrefix}-openssl ${mingwPrefix}-gmp" } -ErrorMessage "MSYS2 dependency install failed."
...
`$ExternalIp = (Invoke-RestMethod -Uri "https://ifconfig.io" -TimeoutSec 10).Trim()
