...
echo "Build required BLS static library variants..."
make -j$(nproc) lib/libbls256.a lib/libbls384.a lib/libbls384_256.a
...
copy_required_lib() {
  ...
  if [ -z "$found" ] || [ ! -f "$found" ]; then
    if [ "$name" = "libbls512.a" ] && [ -n "$backup" ] && [ -f "$backup/libbls512.a" ]; then
      echo "WARNING: $name was not generated by herumi/bls. Restore it from backup: $backup/libbls512.a"
      cp -f "$backup/libbls512.a" "$dst/$name"
      return 0
    fi
